...
function Show-OcrDevices{if(Test-Path $Secrets){$s=Read-Json $Secrets}else{$s=Default-Secrets};Set-OcrEnvironment $s 'ingestion';& $Python $Gateway '--config' (Join-Path $PSScriptRoot 'service\pp-structure-v3-8gb.yaml') '--model-root' (Join-Path $PSScriptRoot 'models') '--jobs-root' (Join-Path $DataRoot 'jobs') '--list-devices';exit $LASTEXITCODE}
