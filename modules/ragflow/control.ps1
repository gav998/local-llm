...
function Verify-Payload{Test-Payload;$env:PATH=(Join-Path $PSScriptRoot 'runtime\vc')+';'+(Split-Path $Python)+';'+$env:SystemRoot+'\System32';& $Python -c 'import cv2, elasticsearch, flask, minio, peewee, quart, valkey';if($LASTEXITCODE -ne 0){throw 'RAG archive import probe failed'};Write-Host '[OK] RAG archive imports verified'}
...
"@|Set-Content (Join-Path $Rag 'conf\local.service_conf.yaml') -Encoding UTF8
 Set-RagEnvironment;& $Python -c 'import cv2, elasticsearch, flask, minio, peewee, quart, valkey';if($LASTEXITCODE -ne 0){throw 'RAGFlow imports failed'};Write-Connection ([ordered]@{schema=1;module='ragflow';url="http://127.0.0.1:$Port"});Set-Installed;Write-Host '[OK] ragflow installed and configured from module contracts'
}
