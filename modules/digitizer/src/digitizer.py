#!/usr/bin/env python3
"""Loopback-only structured PDF digitizer backed by PaddleOCR and llama.cpp."""
...
def main() -> int:
    ...
    html_path = Path(__file__).with_name("digitizer.html")
    registry = SourceRegistry(args.data_root)
    store = DocumentStore()
    tasks = ExtractionTasks(
        store, PaddleClient(args.paddle_url, args.paddle_token), LlamaClient(args.llama_url)
    )
