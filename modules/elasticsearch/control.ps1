...
function Install-Elastic{Begin-Install;$env:ES_JAVA_HOME=Join-Path $Runtime 'jdk';& (Join-Path $Runtime 'bin\elasticsearch.bat') -V;if($LASTEXITCODE -ne 0){throw 'Elasticsearch version probe failed'};Write-ElasticConfig;Write-Connection ([ordered]@{schema=1;module='elasticsearch';url="http://127.0.0.1:$Port"});Set-Installed;Write-Host '[OK] elasticsearch installed'}
