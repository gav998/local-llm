...
function Install-Web{Begin-Install;Write-WebConfig;Write-Connection ([ordered]@{schema=1;module='web';url="http://127.0.0.1:$WebPort"});Set-Installed;Write-Host '[OK] web installed'}
...
switch($CommandName.ToLowerInvariant()){'install'{Install-Web}'start'{Assert-Installed;Start-Web}'stop'{Stop-Web}'status'{Show-ModuleStatus @('web')}'verify'{if((Get-OwnedProcess 'web')-and -not(Test-Http "http://127.0.0.1:$WebPort/")){throw 'Web health failed'};Write-Host '[OK] web verified'}default{Write-Host 'Usage: MODULE.bat install|start|stop|status|verify';if($CommandName -ne 'help' -and $CommandName){exit 2}}}
