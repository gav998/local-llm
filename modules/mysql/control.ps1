...
function Invoke-MySqlQuietly([string]$Executable,[string[]]$Arguments){
 $PreviousErrorActionPreference=$ErrorActionPreference
 try{
  # Expected native failures must remain exit codes. Windows PowerShell otherwise
  # promotes stderr from mysql.exe and mysqladmin.exe to NativeCommandError.
  $ErrorActionPreference='Continue'
  & $Executable @Arguments 2>$null|Out-Null
  return $LASTEXITCODE
 }finally{$ErrorActionPreference=$PreviousErrorActionPreference}
}
function Probe-MySql{return (Invoke-MySqlQuietly (Join-Path $Runtime 'bin\mysql.exe') @("--defaults-extra-file=$ClientIni",'--batch','--skip-column-names','-e','SELECT 1')) -eq 0}
