#!/usr/bin/env python3
"""Portable PaddleOCR API with simple and asynchronous endpoints.
...
def load_pipeline(
    ...
    if not strict_gpu and os.environ.get("LOCAL_OCR_ALLOW_CPU") != "1":
        raise RuntimeError(
            "OCR resolved to CPU. Set LOCAL_OCR_ALLOW_CPU=1 only for an "
            "intentional CPU diagnostic run."
        )
...
def create_app(manager: JobManager, token: str) -> Any:
    ...
    async def enqueue_upload(file: UploadFile, optional_payload: str) -> str:
        try:
    ...
    def simple_result(job_id: str) -> dict[str, Any]:
        pages: list[str] = []
        with manager.result_path(job_id).open(encoding="utf-8") as source:
            for line in source:
                ...
                if not isinstance(text, str) or not text.strip():
                    ...
                    for layout in result.get("layoutParsingResults") or []:
                        for block in (layout.get("prunedResult") or {}).get(
                            "parsing_res_list", []
                        ):
                            content = block.get("block_content")
                            if content:
                    ...
                    text = "\n".join(blocks)
                pages.append(text.strip() if isinstance(text, str) else "")
        return {
    ...
    async def submit_job(
        ...
        job_id = await enqueue_upload(file, optionalPayload)
    ...
    async def recognize(
        ...
        """Accept an image/PDF and return its recognized text in one response."""
        require_token(authorization)
        job_id = await enqueue_upload(file, optionalPayload)
        while True:
            state = manager.status(job_id)
            if state.get("state") == "done":
                return simple_result(job_id)
            if state.get("state") == "failed":
                raise HTTPException(
                    status_code=500,
                    detail=state.get("errorMsg") or "PaddleOCR failed",
                )
