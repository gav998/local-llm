...
При запуске профиля открывается одно отдельное окно с живым объединённым выводом
логов его сервисов. Каждая строка помечена именем сервиса; окно является только
наблюдателем, поэтому его можно закрыть без остановки стека. Окно можно отключить,
запустив BAT с `LOCAL_LLM_LIVE_LOGS=0` в окружении. Полные логи всегда остаются в
`app\logs`.

Общего таймаута startup readiness больше нет: на медленном HDD controller ждёт
готовности столько, сколько потребуется. Падение child/supervisor по-прежнему
обнаруживается сразу. Короткие таймауты одной HTTP/TCP-пробы сохранены, чтобы сама
проверка не могла зависнуть; timeout штатной остановки также остаётся защитным.
