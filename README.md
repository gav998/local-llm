...
После успешного `CHECK-SOURCES.bat` нужно запустить `PREPARE-ONLINE.bat` во всех
восьми каталогах `modules\<name>`. `PREPARE-STACK.bat` проверяет наличие этих
восьми готовых `.7z` и только затем упаковывает отдельную оболочку управления.
...
`LOCAL-LLM.bat install` — offline-команда. Не запускайте её из исходного
online-каталога репозитория: наличие там `MODULE.bat` не означает, что vendor
payload собран. В каталоге установки у каждого модуля должен присутствовать
созданный при сборке `payload.sha256.json`.
