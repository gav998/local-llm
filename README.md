...
Из корня запустите `1.PREPARE-ONLINE.bat` и выберите модуль цифрой. Общее меню
вызывает соответствующий `modules\<name>\PREPARE-ONLINE.bat`. Сценарий покажет
точное имя, неизменённый URL и общий каталог `_src` для каждого отсутствующего
файла. Положите файл туда без переименования и продолжите. Результат появится в
`prepared\local-llm-<name>-<version>.7z`.
...
После успешного `CHECK-SOURCES.bat` нужно через `1.PREPARE-ONLINE.bat` собрать
все восемь модулей. При необходимости модульный `PREPARE-ONLINE.bat` можно
запустить напрямую. `PREPARE-STACK.bat` проверяет наличие восьми готовых `.7z`
и создаёт ZIP оболочки в том же общем каталоге `prepared`.

Для RAGFlow и OCR сохраняются wheelhouse и package cache в `_src\ragflow` и
`_src\paddleocr`; загружаемые исходные файлы лежат непосредственно в `_src`.
...
## Перенос и проверенная распаковка

1. Соберите восемь модульных `.7z` через `1.PREPARE-ONLINE.bat` и запустите
   `PREPARE-STACK.bat`. Все девять архивов появятся в `prepared`.
2. На целевом внешнем NTFS/exFAT-диске создайте отдельный каталог и положите в
   него ZIP оболочки и восемь модульных `.7z`.
3. Вручную распакуйте в этот каталог только `local-llm-stack-2026.09.11.zip`.
4. Запустите `EXTRACT-MODULES.bat`. Он использует поставляемый вместе с
   оболочкой `7za.exe` 26.02, сверяет размер и SHA-256 каждого архива, тестирует
   все архивы до распаковки, распаковывает их и проверяет payload каждого
   модуля. При необходимости каталог архивов можно передать первым аргументом.
5. Запустите `LOCAL-LLM.bat install`. Оболочка лишь последовательно вызывает
   независимый `MODULE.bat install` каждого модуля.

Не распаковывайте модульные архивы старой установленной версией 7-Zip: уже
собранные архивы могут содержать ARM64-фильтр, появившийся в 7-Zip 23.01. Новые
сборки ограничивают методы уровнем совместимости 19.00, но поставляемый
распаковщик остаётся каноническим вариантом. Лог распаковки записывается в
`stack\logs\extract-modules.log`, а команды `LOCAL-LLM.bat` — в
`stack\logs\control.log`.
