...
function Assert-SealedModules([string[]]$Names){
 ...
 if($Missing.Count -gt 0){throw "Offline stack is incomplete: sealed payloads are missing for $($Missing -join ', '). CHECK-SOURCES.bat checks downloads only. Build each module with modules\<name>\PREPARE-ONLINE.bat, then extract the orchestrator ZIP and all eight module .7z archives into a separate deployment directory. Run LOCAL-LLM.bat there, not in the source tree."}
}
