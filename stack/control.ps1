...
$Order=@('mysql','elasticsearch','silo','valkey','llama-cpp','paddleocr','digitizer','ragflow','web')
...
function Start-Chat{Run 'digitizer' 'stop';Run 'ragflow' 'stop' 'task-executor';Run 'paddleocr' 'stop';Run 'llama-cpp' 'stop' 'embedding';try{Start-Data;Run 'llama-cpp' 'start' 'embedding';Run 'llama-cpp' 'start' 'chat';Run 'ragflow' 'start' 'api';Run 'web' 'start'}catch{Run 'llama-cpp' 'stop';throw};Write-Host '`n[OK] chat profile ready: http://127.0.0.1:9388' -ForegroundColor Green}
function Start-PaddleOcr{Run 'digitizer' 'stop';foreach($m in @('web','ragflow','llama-cpp','valkey','silo','elasticsearch','mysql')){Run $m 'stop'};Run 'paddleocr' 'start';Write-Host "[OK] standalone PaddleOCR API: http://127.0.0.1:9399" -ForegroundColor Green}
function Start-Digitizer{foreach($m in @('web','ragflow','valkey','silo','elasticsearch','mysql')){Run $m 'stop'};Run 'llama-cpp' 'stop' 'embedding';try{Run 'paddleocr' 'start';Run 'digitizer' 'start'}catch{Run 'digitizer' 'stop';throw};Write-Host '[OK] structured PDF digitizer: http://127.0.0.1:9400' -ForegroundColor Green}
function Stop-All{foreach($m in @('web','ragflow','digitizer','paddleocr','llama-cpp','valkey','silo','elasticsearch','mysql')){try{Run $m 'stop'}catch{Write-Warning $_}}}
try{switch($CommandName.ToLowerInvariant()){
 ...
 'start'{Assert-Deployment;switch($Profile){'ingestion'{Start-Ingestion}'ingestion-cpu'{Start-IngestionCpu}'chat'{Start-Chat}'paddleocr'{Start-PaddleOcr}'digitizer'{Start-Digitizer}'core'{Start-Core;Write-Host '[OK] core profile ready: http://127.0.0.1:9388' -ForegroundColor Green}default{if([string]::IsNullOrWhiteSpace($Profile)){Start-Core;Write-Host '[OK] core profile ready: http://127.0.0.1:9388' -ForegroundColor Green}else{throw "Unknown start profile '$Profile'. Expected core, digitizer, paddleocr, ingestion, ingestion-cpu or chat."}}}}
 'stop'{Stop-All}
...
Usage:
  ...
  LOCAL-LLM.bat start [core|digitizer|paddleocr|ingestion|ingestion-cpu|chat]
  LOCAL-LLM.bat stop
